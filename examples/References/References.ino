...
void setup()
{
  ...
  RefMatrix<4,4,Array<8,8>> bigMatrixRef(bigMatrix.Submatrix(Range<4>(4),Range<4>(2)));
  ...
  bigMatrix.Submatrix(Range<4>(4),Range<4>(3)) = arr;

  // For all intents and purposes you can treat reference matrices as just regular matrices.
  RefMatrix<2,4,Array<8,8>> anotherRef = bigMatrix.Submatrix(Range<2>(2),Range<4>(1)); // this creates a 2x4 reference matrix starting at element (2,1) of bigMatrix
